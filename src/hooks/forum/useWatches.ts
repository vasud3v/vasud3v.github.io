...
export function useWatches({
    ...
    const markThreadRead = useCallback(async (threadId: string) => {
        ...
        try {
            ...
            if (error) console.error('[useWatches] Failed to mark thread as read:', error);
        } catch (error) {
            console.error('[useWatches] Error marking thread as read:', error);
        }
    }, [currentUser?.id, setCategoriesState]);

    // Cleanup on logout
    const resetWatches = useCallback(() => {
        setWatchedThreads(new Set());
